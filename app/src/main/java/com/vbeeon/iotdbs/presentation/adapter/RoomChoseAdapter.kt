...
class RoomChoseAdapter internal constructor(val context: Context,
    ...
    private var listMenu = emptyList<Room>() // Cached copy of words
    ...
    inner class ViewHolder(itemBinding: ItemRoomChoseBinding) : RecyclerView.ViewHolder(itemBinding.getRoot()) {
        ...
        fun bind(obj: Room?) {
            ...
            var adapterSubSWAdapter : SWAdapter
            adapterSubSWAdapter = SWAdapter(context, doneClick = {

            })
            itemMenuBinding.rcvSW.layoutManager = LinearLayoutManager(context, LinearLayoutManager.VERTICAL ,false)
            itemMenuBinding.rcvSW.apply { adapter = adapterSubSWAdapter }
            itemMenuBinding.rcvSW.isNestedScrollingEnabled = false
            itemMenuBinding.executePendingBindings()
            adapterSubSWAdapter.setDatas(obj!!.listSW)
        }
        init {
