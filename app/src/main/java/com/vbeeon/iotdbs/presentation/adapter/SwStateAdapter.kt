...
import com.vbeeon.iotdbs.databinding.ItemSwStateBinding
...
class SwStateAdapter internal constructor(val context: Context,
                                          val doneClick: (Int) -> Unit) : RecyclerView.Adapter<SwStateAdapter.ViewHolder>(){
    private var listMenu = emptyList<SwitchDetailEntity>() // Cached copy of words

    inner class ViewHolder(itemBinding: ItemSwStateBinding) : RecyclerView.ViewHolder(itemBinding.getRoot()) {
        var itemMenuBinding: ItemSwStateBinding
        fun bind(obj: SwitchDetailEntity?) {
            itemMenuBinding.data = obj
            itemMenuBinding.executePendingBindings()
            if (obj!!.isChecked){
                itemMenuBinding.imgSWState.setImageDrawable(context.getDrawable(R.drawable.ic_switch_detail_on))
            }else
                itemMenuBinding.imgSWState.setImageDrawable(context.getDrawable(R.drawable.ic_switch_detal_off))
        }
        init {
    ...
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): ViewHolder {
        val binding: ItemSwStateBinding = DataBindingUtil.inflate(LayoutInflater.from(parent.context),
                R.layout.item_sw_state, parent, false)
