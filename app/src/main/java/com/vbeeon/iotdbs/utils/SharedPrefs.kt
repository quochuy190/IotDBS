...
class SharedPrefs private constructor(){
    ...
    init {
        mSharedPreferences = IotDBSApplication.instance!!.getSharedPreferences(PREFS_NAME, Context.MODE_PRIVATE)
    }
    ...
    operator fun <T> get(key: String, anonymousClass: Class<T>): T {
        return when (anonymousClass) {
            ...
            else -> IotDBSApplication.instance?.gSon?.fromJson(mSharedPreferences.getString(key, ""), anonymousClass)!!
        }
    }
    ...
    fun <T> put(key: String, data: T) {
        ...
        when (data) {
            ...
            else -> editor.putString(key, IotDBSApplication.instance?.gSon?.toJson(data))
        }
        editor.apply()
    }
